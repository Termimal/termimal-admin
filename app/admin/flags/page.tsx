...
export default function FlagsPage() {
  ...
  const load = async () => {
    ...
    const { data, error } = await supabase
      .from('feature_flags')
      .select('id, name, description, environment, enabled, min_plan')
  ...
  if (loading) return <div style={{ color: 'var(--t3)' }}>Loading flags...</div>

  return (
    <div className="max-w-6xl">
      <div className="mb-8">
        <h1 className="text-2xl font-bold tracking-tight mb-1" style={{ color: 'var(--t1)' }}>Feature Flags</h1>
        <p className="text-sm" style={{ color: 'var(--t3)' }}>Toggle feature availability by environment and plan.</p>
      </div>
            ...
            {flags.map(f => (
              <tr key={f.id} style={{ borderBottom: '1px solid var(--border)' }}>
                ...
                <td className="px-4 py-2.5">
                  <button onClick={() => toggleFlag(f)} disabled={savingId === f.id} className="w-9 h-5 rounded-full relative transition-all disabled:opacity-60" style={{ background: f.enabled ? 'var(--acc2)' : 'var(--border)' }} aria-label={`Toggle ${f.name}`}>
